...
The program enters protected mode and executes `MULU`. On CPU models without coprocessor 3, the invalid-opcode handler runs in supervisor mode, dispatches from the command stored in the fault metadata register, retargets the saved return state to a normal supervisor-mode trampoline, and returns from the fault. The trampoline calculates the unsigned product with a software shift-and-add routine, restores the original protected-mode return state, and resumes the protected program as though the hardware operation had completed.
