...
\begin{description}
	...
	\item[When Set (1):] CPU operates in protected mode with restricted access to:
	      ...
	      \end{itemize}
	\item[Note:] This bit provides basic memory protection by preventing user code from changing memory segments. With the exception of writing to the status register, preforming a restricted operation will cause a "privilege violation" fault.
\end{description}

\subsection{Interrupt Mask Bits (I1, I2, I3) -- Bits 9--11}

% TODO: The exception handling should probably get its own chapter? explain different types of exceptions and priorities etc. and the sliding link registers
% TODO: Clarify interrupt vs exception
% TODO: Is there any way to disable specific hardware interrupt levels without blocking software exceptions? I don't think so. Could this be an issue? Maybe leave that up to an external chip?
...
%Interrupt handling is discussed in detail in Chapter~\ref{ch:interrupts}.
...
\begin{description}
	...
	\item[Note:] This bit is primarily for hardware implementation, and is mapped to an external pin. No program will ever read this bit as 1 as the CPU is not executing when this bit is active.
\end{description}
