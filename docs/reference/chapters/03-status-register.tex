...
If an exception comes in with a lower priority than the mask, it will not be serviced until the higher priority
interrupts are finished.
...
If a fault occurs while handling another fault the CPU will be halted. This is due to the architecture of the exception
unit.
...
\begin{table}[H]
	...
	\begin{tabular}{cp{7cm}}
		\toprule
		\textbf{Value} & \textbf{Interrupt Type} \\
		\midrule
		0x0            & No Interrupt            \\
...
\begin{description}
	...
	\item[Note:] This bit is primarily for hardware implementation, and is mapped to an external pin. No program will ever read
	      this bit as 1 as the CPU is not executing when this bit is active.
\end{description}
