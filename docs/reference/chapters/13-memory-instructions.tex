...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		\toprule
		\textbf{Instruction} & \textbf{Immediate Offset} & \textbf{Register Offset}  \\
		\midrule
		LOAD & \texttt{LOAD rD, (\#offset, addr)} & \texttt{LOAD rD, (rO, addr)[, shift]} \\
		STOR & \texttt{STOR (\#offset, addr), rS} & \texttt{STOR (rO, addr), rS[, shift]} \\
		\bottomrule
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{lll}
		...
		\bottomrule
	\end{tabular}
	\caption{Legal Memory Auto-update Forms}
	\label{tab:memory-auto-update-forms}
\end{table}
