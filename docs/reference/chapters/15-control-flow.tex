...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		...
		LJSR                 & \texttt{LJSR (\#offset, src)+}       & \texttt{LDEL} alias & Assembles to \texttt{LDEL p, (\#offset, src)+} \\
		LJSR                 & \texttt{LJSR (rS, src)+}             & \texttt{LDEL} alias & Assembles to \texttt{LDEL p, (rS, src)+}       \\
		\bottomrule
	\end{tabular}
...
\end{lstlisting}
	...
	Branches relative to the current program counter. \mnemonic{BRAN} is an assembler alias, not a distinct CPU opcode. Use
	\mnemonic{LDEA p, (\#offset, src)} or \mnemonic{LDEA p, (rS, src)} for non-PC-relative computed jumps.
