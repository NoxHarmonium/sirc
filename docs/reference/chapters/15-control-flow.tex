...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		...
		LDEA                 & \texttt{LDEA dest, (rO, src)}        & 0x19               & Register-offset effective address                                           \\
		LDEA                 & \texttt{LDEA dest, -(\#offset, src)} & 0x1A               & Pre-decrement source, immediate offset                                      \\
		LDEA                 & \texttt{LDEA dest, -(rO, src)}       & 0x1B               & Pre-decrement source, register offset                                       \\
		LDEL                 & \texttt{LDEL dest, (\#offset, src)}  & 0x1C               & Immediate-offset effective address and link; \texttt{dest} excludes \reg{l} \\
		LDEL                 & \texttt{LDEL dest, (rO, src)}        & 0x1D               & Register-offset effective address and link; \texttt{dest} excludes \reg{l}  \\
		LDEL                 & \texttt{LDEL dest, (\#offset, src)+} & 0x1E               & Post-increment source, immediate offset; \texttt{dest} excludes \reg{l}     \\
		LDEL                 & \texttt{LDEL dest, (rO, src)+}       & 0x1F               & Post-increment source, register offset; \texttt{dest} excludes \reg{l}      \\
		...
		LJMP                 & \texttt{LJMP src, rO}                & \texttt{LDEA} meta & Assembles to \texttt{LDEA p, (rO, src)}                                     \\
		...
		LJSR                 & \texttt{LJSR src, rO}                & \texttt{LDEL} meta & Assembles to \texttt{LDEL p, (rO, src)}                                     \\
		LJSR                 & \texttt{LJSR (\#offset, src)+}       & \texttt{LDEL} meta & Assembles to \texttt{LDEL p, (\#offset, src)+}                              \\
		LJSR                 & \texttt{LJSR (rO, src)+}             & \texttt{LDEL} meta & Assembles to \texttt{LDEL p, (rO, src)+}                                    \\
		\bottomrule
	\end{tabular}
...
pair. \reg{l} is never a legal \mnemonic{LDEL} destination, because \mnemonic{LDEL} always writes the return address to
\reg{l}; the destination and the link write would alias. \mnemonic{LDEL} destinations are \reg{a}, \reg{s}, or \reg{p}.

\noindent\textbf{Zero displacement:} Immediate-displacement forms may omit \texttt{\#0}. Thus
\texttt{LDEA dest, (src)}, \texttt{LDEA dest, -(src)}, \texttt{LDEL dest, (src)}, and
\texttt{LDEL dest, (src)+} assemble as the matching \texttt{\#0} forms. For non-auto-update absolute jumps and calls,
use the meta-instruction syntax \texttt{LJMP src} or \texttt{LJSR src}; \texttt{LJMP (src)} and \texttt{LJSR (src)} are
not public syntax. The post-increment call shorthand \texttt{LJSR (src)+} is legal and assembles as
\texttt{LDEL p, (\#0, src)+}.

\noindent\textbf{Aliased register writes:} Instructions that write the same address-register pair through more than one
write-back path have architecturally undefined behavior. The assembler rejects exactly these forms: \mnemonic{LDEA}
pre-decrement with destination equal to source; \mnemonic{LDEL} with destination \reg{l} (any form); \mnemonic{LDEL}
post-increment with source \reg{l} or with destination equal to source; \mnemonic{LJSR} post-increment from \reg{l} or
\reg{p}; \mnemonic{LOAD} post-increment whose destination is a half of the auto-updated pair; and \mnemonic{STOR}
pre-decrement whose source is a half of the auto-updated pair.
...
Control-flow and effective-address instructions do not update status flags. All four condition flags are preserved
regardless of whether the instruction branches, links, faults, or is skipped. Control-flow instructions do not support
status update override syntax.
...
\begin{table}[H]
	...
	\begin{tabular}{lllllp{5.5cm}}
		...
		\midrule
		LDEA                 & -          & -          & -          & -          & Flags are preserved. \\
		LDEL                 & -          & -          & -          & -          & Flags are preserved. \\
		\bottomrule
	\end{tabular}
	\caption{Control-Flow Instruction Status Flag Effects}
	\label{tab:control-flow-status-flag-effects}
\end{table}

See Table~\ref{tab:flag-effect-symbols} for the flag-effect symbol definitions. Meta-instructions (\mnemonic{BRAN},
\mnemonic{BRSR}, \mnemonic{RETS}, \mnemonic{LJMP}, \mnemonic{LJSR}) inherit the flag-preservation behavior of the
\mnemonic{LDEA} or \mnemonic{LDEL} form they assemble to.
...
\begin{instructionbox}{BRAN}{Branch Meta-Instruction}
	...
	\textbf{Assembles to:} \mnemonic{LDEA} with \reg{p} as the source and destination. See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.

	\textbf{Instruction Fields:}
	\begin{itemize}
		\item \textbf{Register field (bits 25--22, low 2 bits used):} fixed at \texttt{0x3} (\reg{p}) by the
		      meta-instruction.
		\item \textbf{AF (bits 5--4):} fixed at \texttt{0x3} (\reg{p}) by the meta-instruction.
		\item \textbf{Immediate (bits 21--6):} signed 16-bit displacement.
...
\end{lstlisting}
	...
	\textbf{Operands:} \texttt{\#disp} is a signed 16-bit word displacement (\texttt{-32768} to \texttt{+32767}) added to
	the low word of \reg{p} modulo $2^{16}$; a branch cannot change the program segment, so the displacement wraps within
	the current 64K segment. \texttt{@label} is resolved by the linker to a word displacement from the address of the
...
\end{verbatim}
	...
	opcode. The displacement base is the branching instruction's own address, so \texttt{BRAN \#0} branches to itself and
	\texttt{BRAN \#2} falls through to the next instruction. Use \mnemonic{LDEA p, (\#offset, src)} or \mnemonic{LDEA p,
		(rO, src)} for non-PC-relative computed jumps.
	...
	\textbf{Condition field:} If the condition is false, no branch occurs and status flags are preserved.
...
loop:
    ...
    BRAN|!= @loop             ; Jump back if not zero
		\end{lstlisting}
		\caption{Unconditional and Conditional Branching}
	\end{example}
...
\begin{instructionbox}{BRSR}{Branch to Subroutine Meta-Instruction}
	...
	\textbf{Assembles to:} \mnemonic{LDEL} with \reg{p} as the source and destination. See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.
...
\end{lstlisting}
	...
	\textbf{Operands:} \texttt{\#disp} is a signed 16-bit word displacement (\texttt{-32768} to \texttt{+32767}) added to
	the low word of \reg{p} modulo $2^{16}$; a call cannot change the program segment, so the displacement wraps within
	the current 64K segment. \texttt{@label} is resolved by the linker to a word displacement from the address of the
...
\end{verbatim}
	...
	Calls a PC-relative subroutine by saving the return address in \reg{l} and branching to the computed target. The
	displacement base is the branching instruction's own address, so the \mnemonic{BRSR} link value is that address plus 2
	(the address of the next instruction). Return with \mnemonic{RETS}, which assembles to \texttt{LDEA p, (\#0, l)}.
	...
	\textbf{Condition field:} If the condition is false, no call occurs, \reg{l} is not written, and status flags are
	preserved.
...
RETS
		\end{lstlisting}
		\caption{Calling and Returning with BRSR/RETS}
	\end{example}
...
\begin{instructionbox}{RETS}{Return from Subroutine Meta-Instruction}
	...
	\textbf{Assembles to:} \texttt{LDEA p, (\#0, l)}. See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.

	\textbf{Instruction Fields:}
	\begin{itemize}
		\item \textbf{Register field (bits 25--22, low 2 bits used):} fixed at \texttt{0x3} (\reg{p}) by the
		      meta-instruction.
		\item \textbf{AF (bits 5--4):} fixed at \texttt{0x0} (\reg{l}) by the meta-instruction.
		\item \textbf{Immediate (bits 21--6):} fixed at \texttt{0x0000} by the meta-instruction.
		\item \textbf{Condition field (bits 3--0):} any of the 16 condition-code encodings in
		      Table~\ref{tab:condition-code-encoding}.
	\end{itemize}
...
\end{verbatim}
	...
	\textbf{Condition field:} If the condition is false, no return occurs and status flags are preserved.
...
my_function:
    ...
    RETS                      ; Return to caller
		\end{lstlisting}
		\caption{Returning from a Subroutine}
	\end{example}
...
\begin{instructionbox}{LDEA}{Load Effective Address}
	...
	\textbf{Opcodes:} 0x18 (Immediate), 0x19 (Register), 0x1A (Pre-Dec Immediate), 0x1B (Pre-Dec Register). See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.

	\textbf{Instruction Fields:}
	\begin{itemize}
		\item \textbf{Register field (bits 25--22, low 2 bits used):} destination address-register pair \texttt{dest};
		      \texttt{0x0} = \reg{l}, \texttt{0x1} = \reg{a}, \texttt{0x2} = \reg{s}, \texttt{0x3} = \reg{p}.
		\item \textbf{AF (bits 5--4):} source address-register pair \texttt{src}; \texttt{0x0} = \reg{l}, \texttt{0x1} =
		      \reg{a}, \texttt{0x2} = \reg{s}, \texttt{0x3} = \reg{p}.
		\item \textbf{Immediate (bits 21--6, immediate forms only):} 16-bit signed displacement.
		\item \textbf{R3 (bits 17--14, register forms only):} \texttt{rO}, a general-purpose register supplying the
		      displacement.
		\item \textbf{Condition field (bits 3--0):} any of the 16 condition-code encodings in
		      Table~\ref{tab:condition-code-encoding}.
	\end{itemize}
...
LDEA dest, (src)              ; equivalent to LDEA dest, (#0, src)
LDEA dest, (rO, src)          ; dest = src + rO
LDEA dest, -(#offset, src)    ; src -= 1; dest = src + offset
LDEA dest, -(src)             ; equivalent to LDEA dest, -(#0, src)
LDEA dest, -(rO, src)         ; src -= 1; dest = src + rO
LDEA|cond dest, (#offset, src)
\end{lstlisting}

	\textbf{Operands:} \texttt{dest} and \texttt{src} are address-register pairs. Immediate forms use a 16-bit
	displacement. Register forms use \texttt{rO} as a displacement. Pre-decrement forms also update \texttt{src}.
...
\end{verbatim}
	...
	\textbf{Condition field:} If the condition is false, no destination or source address register is written and status
	flags are preserved.
...
LDEA a, -(#0, s)              ; s -= 1; a = s
		\end{lstlisting}
		\caption{Pointer Arithmetic and Computed Jumps with LDEA}
	\end{example}
...
\begin{instructionbox}{LDEL}{Load Effective Address and Link}
	...
	\textbf{Opcodes:} 0x1C (Immediate), 0x1D (Register), 0x1E (Post-Inc Immediate), 0x1F (Post-Inc Register). See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.

	\textbf{Instruction Fields:}
	\begin{itemize}
		\item \textbf{Register field (bits 25--22, low 2 bits used):} destination address-register pair \texttt{dest};
		      \texttt{0x1} = \reg{a}, \texttt{0x2} = \reg{s}, \texttt{0x3} = \reg{p} (\texttt{0x0} = \reg{l} is not a legal
		      \mnemonic{LDEL} destination).
		\item \textbf{AF (bits 5--4):} source address-register pair \texttt{src}; \texttt{0x0} = \reg{l}, \texttt{0x1} =
		      \reg{a}, \texttt{0x2} = \reg{s}, \texttt{0x3} = \reg{p}.
		\item \textbf{Immediate (bits 21--6, immediate forms only):} 16-bit signed displacement.
		\item \textbf{R3 (bits 17--14, register forms only):} \texttt{rO}, a general-purpose register supplying the
...
LDEL dest, (src)              ; equivalent to LDEL dest, (#0, src)
LDEL dest, (rO, src)          ; l = next PC; dest = src + rO
LDEL dest, (#offset, src)+    ; l = next PC; dest = src + offset; src += 1
LDEL dest, (src)+             ; equivalent to LDEL dest, (#0, src)+
LDEL dest, (rO, src)+         ; l = next PC; dest = src + rO; src += 1
LDEL|cond dest, (#offset, src)
\end{lstlisting}

	\textbf{Operands:} \texttt{dest} and \texttt{src} are address-register pairs. Immediate forms use a 16-bit
	displacement. Register forms use \texttt{rO} as a displacement. Post-increment forms also update \texttt{src}.
...
\end{verbatim}
	...
	\textbf{Condition field:} If the condition is false, no destination, link, or source address register is written and
...
LDEL p, (#0, a)+              ; l = next PC; p = old a; a += 1
		\end{lstlisting}
		\caption{Calling Through a Register with LDEL}
	\end{example}
...
\begin{instructionbox}{LJMP}{Long Jump Meta-Instruction}
	...
	\textbf{Assembles to:} \mnemonic{LDEA} with \reg{p} as the destination. See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.
	...
	\begin{itemize}
		...
		\item \textbf{AF (bits 5--4):} source address-register pair \texttt{src}; \texttt{0x0} = \reg{l}, \texttt{0x1} =
		      \reg{a}, \texttt{0x2} = \reg{s}, \texttt{0x3} = \reg{p}.
		\item \textbf{Immediate (bits 21--6, \texttt{\#offset} form only):} 16-bit signed displacement; fixed at
		      \texttt{0x0000} for plain \texttt{LJMP src}.
		\item \textbf{R3 (bits 17--14, \texttt{rO} form only):} \texttt{rO}, a general-purpose register supplying the
		      displacement.
		\item \textbf{Condition field (bits 3--0):} any of the 16 condition-code encodings in
		      Table~\ref{tab:condition-code-encoding}.
	\end{itemize}
...
LJMP src, rO                  ; p = src + rO
LJMP|cond src                 ; Conditional long jump
\end{lstlisting}

	\textbf{Operands:} \texttt{src} is an address-register pair. Optional \texttt{\#offset} or \texttt{rO} operands select
...
\end{verbatim}
	...
	\textbf{Condition field:} If the condition is false, no jump occurs and status flags are preserved.
...
LJMP a, r1                     ; p = a + r1
		\end{lstlisting}
		\caption{Jumping Through a Register with LJMP}
	\end{example}
...
\begin{instructionbox}{LJSR}{Long Jump to Subroutine Meta-Instruction}
	...
	\textbf{Assembles to:} \mnemonic{LDEL} with \reg{p} as the destination. See Table~\ref{tab:control-flow-legal-forms} for the full family of legal forms.
	...
	\begin{itemize}
		\item \textbf{Register field (bits 25--22, low 2 bits used):} fixed at \texttt{0x3} (\reg{p}) by the
		      meta-instruction.
		\item \textbf{AF (bits 5--4):} source address-register pair \texttt{src}; \texttt{0x0} = \reg{l}, \texttt{0x1} =
		      \reg{a}, \texttt{0x2} = \reg{s}, \texttt{0x3} = \reg{p} for non-post-increment forms. Post-increment forms
		      accept only \texttt{0x1} = \reg{a} or \texttt{0x2} = \reg{s}; \reg{l} and \reg{p} are aliased
		      address-register writes with architecturally undefined behavior and are rejected by the assembler.
		\item \textbf{Immediate (bits 21--6, \texttt{\#offset} forms only):} 16-bit signed displacement; fixed at
		      \texttt{0x0000} for plain \texttt{LJSR src}.
		\item \textbf{R3 (bits 17--14, \texttt{rO} forms only):} \texttt{rO}, a general-purpose register supplying the
		      displacement.
		\item \textbf{Condition field (bits 3--0):} any of the 16 condition-code encodings in
		      Table~\ref{tab:condition-code-encoding}.
	\end{itemize}
...
LJSR src, rO                  ; l = next PC; p = src + rO
LJSR (#offset, src)+          ; l = next PC; p = src + offset; src += 1
LJSR (src)+                   ; equivalent to LJSR (#0, src)+
LJSR (rO, src)+               ; l = next PC; p = src + rO; src += 1
...
\end{verbatim}
	...
	and \reg{p}, post-increment source registers must not be \reg{l} or \reg{p}; such forms are aliased address-register
	writes with architecturally undefined behavior and are rejected by the assembler (see
	...
	\textbf{Condition field:} If the condition is false, no call occurs, \reg{l} is not written, any post-increment source
...
; supervisor mode only -- writing ah is privileged
LOAD ah, #0x0040
...
LJSR (r1, a)+                 ; p = a + r1; a += 1
		\end{lstlisting}
		\caption{Calling Through a Register with LJSR}
	\end{example}
...
; Switch on r1 (0-3)
LOAD r2, r1                   ; r2 = r1
; Scale the index to a 2-word BRAN entry and skip over this LDEA
ADDI r2, #2, LSL #1           ; r2 = (r2 << 1) + 2
LDEA p, (r2, p)               ; Jump into the branch table below
