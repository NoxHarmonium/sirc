...
\end{lstlisting}
	...
	% TODO: Document how it is different to ADDI/ADDR with pl - it isn't really except that it doesn't update the status register (which you could do anyway by setting the AF to 0b00), and it uses the address unit rather than the ALU which may matter in the future
	% Also: I guess semantically it is different, so in the future if we have pipelining or something maybe it will matter?
	\textbf{Examples:}
	\begin{lstlisting}
...
\begin{lstlisting}
; Switch on r1 (0-3)
ADDR r2, r1, #0, LSL #1       ; r2 = (r1 << 1) = r1 * 2 (word size)
