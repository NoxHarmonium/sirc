...
When asserted, the CPU samples the Level 5 hardware exception line. Unlike \texttt{IRQ1}--\texttt{IRQ4}, which are
level-sensitive, NMI is edge-triggered: it fires once on assertion regardless of how long the pin is held, and the pin
must be released and reasserted to fire again.
...
Because NMI is edge-triggered, holding the pin asserted after its handler returns does not by itself cause it to fire
again -- a fresh falling-then-rising transition is required. Therefore, it is important that the level 5 exception
handlers are executed quickly and that NMI is only asserted for important non-interruptible events.
...
A bus transaction terminated by Bus Error or Bus Protection Error is architecturally unsuccessful. An external device
must not commit any externally visible effect from a transaction for which it asserts the corresponding error signal:
for a write, the device must not act on the data it was driven; for a read, any side effect the access would otherwise
have (for example, a status register that clears on read) must not occur either. This is what makes Bus Fault and Bus
Protection Fault safe to retry (Chapter~\ref{ch:exceptions}) -- the CPU can assume that a transaction it aborted with
one of these signals never happened.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		IRQ1--IRQ4       & Level-sensitive. A line asserted while enabled is latched at that moment, even if released before the next instruction boundary; the exception unit dispatches the latched request at that boundary. \\
		NMI              & Edge-triggered. Fires once on assertion regardless of how long the pin is held; the pin must be released and reasserted to fire again.                                                             \\
