...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llclX}
		...
		0x2         & DMA Unit             & No                & 0x8--0xA            & Optional standard DMA transfer unit                  \\
...
The optional DMA unit provides bulk word transfers using the existing address register pairs and the general-purpose
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llllX}
		...
		\midrule
		\mnemonic{DMAR}   & 0x2800 | n       & 0--7           & \reg{a}, \reg{r1}--\reg{r7}          & Read memory at \reg{a} into registers       \\
		\mnemonic{DMAW}   & 0x2900 | n       & 0--7           & \reg{a}, \reg{r1}--\reg{r7}          & Write registers to memory at \reg{a}        \\
...
\begin{instructionbox}{DMAR -- DMA Read To Registers Meta-Instruction}
	\textbf{Assembles to:} \texttt{COPI \#0x2800 | n}

	\textbf{Syntax:} \texttt{DMAR \#n}

	\textbf{Operands:} \texttt{\#n} is an 8-bit literal count operand in the range 0--7. Counts above 7 are invalid DMA
	operations.

	\textbf{Description:} Reads sequential words from the address in \reg{a} into \reg{r1} through \reg{rn}.
...
\end{lstlisting}
	...
	\textbf{Write-back:} A successful non-zero \mnemonic{DMAR} advances \reg{a} by the number of words transferred.
	Address-register high words are not changed by the transfer increments.

	\textbf{Clobbers:} Writes \reg{r1} through \reg{rn}.

	\textbf{Flags:} Preserved.

	\textbf{Exceptions:} Missing DMA hardware, unsupported DMA operations, and invalid counts raise invalid-opcode faults.
	In protected mode, raises a privilege-violation fault. DMA reads can raise data bus, bus-protection, and
...
\begin{instructionbox}{DMAW -- DMA Write From Registers Meta-Instruction}
	\textbf{Assembles to:} \texttt{COPI \#0x2900 | n}

	\textbf{Syntax:} \texttt{DMAW \#n}

	\textbf{Operands:} \texttt{\#n} is an 8-bit literal count operand in the range 0--7. Counts above 7 are invalid DMA
	operations.

	\textbf{Description:} Writes \reg{r1} through \reg{rn} to sequential words starting at the address in \reg{a}.
...
\end{lstlisting}
	...
	\textbf{Write-back:} A successful non-zero \mnemonic{DMAW} advances \reg{a} by the number of words transferred.
	Address-register high words are not changed by the transfer increments.
	...
	\textbf{Exceptions:} Missing DMA hardware, unsupported DMA operations, and invalid counts raise invalid-opcode faults.
	In protected mode, raises a privilege-violation fault. DMA writes can raise data bus, bus-protection, and
	segment-overflow faults.
...
\begin{instructionbox}{DMAT -- DMA Memory Transfer Meta-Instruction}
	...
	\textbf{Syntax:} \texttt{DMAT \#n}

	\textbf{Operands:} \texttt{\#n} is an 8-bit literal count operand in the range 0--255. A count of zero is a no-op.
...
\end{lstlisting}
	...
	\textbf{Exceptions:} Missing DMA hardware and unsupported DMA operations raise invalid-opcode faults. In protected
	mode, raises a privilege-violation fault. DMA reads and writes can raise data bus, bus-protection, and
	segment-overflow faults.
