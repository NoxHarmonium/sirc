...
selected coprocessor then interprets the command value. If the selected coprocessor is not present in the CPU model,
the exception unit raises an invalid-opcode fault so the operation can be emulated or rejected by software.
...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		...
		COPI                 & \texttt{COPI \#imm8}  & 0x2F              & Zero-extended 8-bit command operand      \\
...
\noindent The CPU encoding reserves register and shift fields inherited from the immediate, short-immediate, and
register instruction formats. The programmer-facing \mnemonic{COPI} and \mnemonic{COPR} syntax hides those fields;
assemblers emit canonical values and coprocessors must not interpret them as destination-register writes.
...
\begin{table}[H]
	...
	\begin{tabular}{lp{9cm}}
		...
		Command operand    & Immediate \mnemonic{COPI} uses the 16-bit immediate value. Short-immediate \mnemonic{COPI} uses the zero-extended encoded 8-bit immediate value as the command operand. \mnemonic{COPR} uses its source register value as the command operand. \\
		...
		Exceptions         & Supervisor-only coprocessor operations raise a privilege-violation fault in protected mode. Missing, invalid, or reserved coprocessor IDs and operations raise invalid-opcode faults.                                                          \\
...
\begin{instructionbox}{COPI/COPR -- Coprocessor Call}
	\textbf{Opcodes:} 0x0F (Immediate), 0x2F (Short Immediate), 0x3F (Register)

	\textbf{Syntax:}
	\begin{lstlisting}
COPI #opcode                  ; Call coprocessor with 16-bit command
COPI #value                   ; Call coprocessor with 8-bit command operand
...
\end{lstlisting}
	...
	\textbf{Exceptions:} May raise privilege-violation or invalid-opcode faults.
