...
The SIRCULAR CPU provides sixteen 16-bit registers that serve various purposes. These registers can be classified into
three categories:
...
The SIRCULAR provides four address register pairs, each consisting of a high and low 16-bit register. When used
together, these pairs form 24-bit addresses by combining the low 8 bits of the high register with the full 16 bits of
the low register.
...
The program counter register pair points to the next instruction to be executed. It is automatically incremented by 4
after each instruction fetch (since instructions are 32 bits).
...
Some instructions (\mnemonic{LDEA}, \mnemonic{LJSR}) that operate on full address register pairs can write to the low
register only when the second operand bit is set. This allows controlled updates to addresses within the current
segment even in protected mode.
