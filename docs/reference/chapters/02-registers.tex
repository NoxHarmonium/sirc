...
The SIRCULAR CPU provides sixteen 16-bit registers that serve various purposes. These registers can be classified into three categories:
...
The SIRCULAR provides seven general-purpose registers:
...
The SIRCULAR provides four address register pairs, each consisting of a high and low 16-bit register. When used together, these pairs form 24-bit addresses by combining the low 8 bits of the high register with the full 16 bits of the low register.
...
The link register pair stores return addresses for subroutine calls. When a subroutine is called using \mnemonic{LJSR} or \mnemonic{BRSR}, the return address is automatically saved to this register pair.

\textbf{Typical Use:}
\begin{itemize}
    \item Storing return addresses from subroutine calls
    \item Building nested subroutine call stacks
\end{itemize}
...
The program counter register pair points to the next instruction to be executed. It is automatically incremented by 4 after each instruction fetch (since instructions are 32 bits).
...
The status register (\reg{sr}) is a special 16-bit register that contains CPU status flags and control bits. It is divided into two bytes:
...
\subsection{Bypassing Restrictions}

Some instructions (\mnemonic{LDEA}, \mnemonic{LJSR}) that operate on full address register pairs can write to the low register only when the second operand bit is set. This allows controlled updates to addresses within the current segment even in protected mode.
...
; Pop r2 from stack (post-increment)
LOAD r2, (#0, s)+
\end{lstlisting}
