...
\begin{table}[H]
	...
	\begin{tabular}{lp{9cm}}
		...
		Fault side effects  & Destination register writes and address-register auto-updates occur in write-back after the memory-access phase, so a fault that aborts execution before write-back leaves them undone. Data bus, data bus-protection, and privilege-violation faults are retryable: the saved return address is the address of the faulting instruction, so returning from the fault handler with \mnemonic{RETE} retries the same memory access. Segment-overflow is the exception: it is retryable only when raised by program-counter wraparound, which cannot happen during a memory instruction's effective-address calculation, so a segment-overflow fault raised here is never retryable and resumes after the faulting access (see Chapter~\ref{ch:exceptions}). \\
		\bottomrule
	\end{tabular}
...
fault is not raised by program-counter wraparound, so it is not retryable: the saved return address is the address of
the next instruction (see Chapter~\ref{ch:exceptions}). A memory instruction raises a privilege-violation fault in
