...
\begin{instructionbox}{LOAD}{Load from Memory}
	...
	\textbf{Opcodes:} \opcode{14} (Indirect Imm), \opcode{15} (Indirect Reg), \opcode{16} (Post-Inc Imm), \opcode{17} (Post-Inc Reg). See Table~\ref{tab:memory-legal-forms} for the full family of legal forms.
...
\end{verbatim}
	...
	segment-overflow fault during effective-address calculation when \texttt{SR.A} is set; this fault is
...
LOAD r3, (r2, a)               ; r3 = memory[a + r2]
		\end{lstlisting}
		\caption{Indirect, Post-Increment, and Register-Offset Loads}
	\end{example}
...
\begin{instructionbox}{STOR}{Store to Memory}
	...
	\textbf{Opcodes:} \opcode{10} (Indirect Imm), \opcode{11} (Indirect Reg), \opcode{12} (Pre-Dec Imm), \opcode{13} (Pre-Dec Reg). See Table~\ref{tab:memory-legal-forms} for the full family of legal forms.
...
STOR -(#0, s), r4             ; s -= 1; memory[s] = r4
		\end{lstlisting}
		\caption{Indirect, Shifted, and Pre-Decrement Stores}
	\end{example}
