...
When low, the CPU immediately aborts the current instruction or bus wait, stops normal processing, resets to phase 0,
and begins the reset sequence described in Chapter~\ref{ch:exceptions}.

The RSTO pin will also be asserted.

When this pin becomes high, the CPU completes the reset-output hold, stops asserting the RSTO pin, and the exception
...
be managed externally to the CPU and RSTI will need to be asserted for however long is needed for the
