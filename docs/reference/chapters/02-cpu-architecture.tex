...
instruction execution phases. It decodes instructions and generates control signals that orchestrate data movement
...
44 & GND2  & Ground \\
45 & CLKI  & Clock input \\
46 & SYNC  & Instruction-complete strobe output \\
...
All inputs are active low, all outputs are active high.
...
These 24 pins declare the address on the bus that the CPU will read or write from when BAS
(bus access strobe) is asserted.
...
These 16 pins will be driven to a 16-bit value when the CPU is outputting to the bus, and will
be an input for a 16-bit value when the CPU is reading from the bus.
...
Each transition from low to high will progress the CPU internal six stage sequence, and so
every six clock transitions will be a full instruction execution (assuming no wait for bus devices).


\subsubsection{Bus Access Strobe Output (SYNC)}
...
When the CPU completes a full instruction (six internal cycles) it will transition this pin from
low to high to signal external devices.

It will be high for cycle 1, and low for every other cycle.
...
When the CPU is ready to perform a bus operation (i.e. the address pins, data pins and BRW pin are
all in the right state) it will transition this pin from low to high.

It will then not progress to the next cycle until the bus asserts the BACK (Bus acknowledge input)
pin, allowing slow devices time to complete operations.


\subsubsection{Bus Access Strobe Output (BRW)}
...
When low, the CPU is requesting a read from the bus. Otherwise, when high, the CPU is requesting
to write to the bus.
...
When low, the bus is asserting that the previously requested read/write was successful, allowing the
CPU I/O operation to complete.

If all bus devices are fast and can complete reads/writes in a single cycle, this pin can be
...
When low, the bus is asserting that the previously requested read/write was unsuccessful.

This will cause the CPU to abort the I/O operation and raise a bus fault exception.

An example of a situation where a bus error might be raised would be I/O to an address that is not
...
When low, the bus is asserting that the previously requested read/write was to a valid
address/device, but that the I/O is disallowed given the current state of the CPU or bus.

This will cause the CPU to abort the I/O operation and raise a bus protection fault exception.

It acts very similar to the BERR pin but raises a different fault so that programs can handle it
differently.

An example of a situation where a bus protection error might be raised would be a write to ROM, or
a write to a privileged section of memory when the CPU is not in supervisor mode.
...
For example, if a memory controller knows that the DMA co-processor will be reading more than
one sequential address, it can prefetch the next address.

Another example might be a memory controller providing memory protection if these pins are used in
conjuction with the PROT (Protected-mode output) pin. A memory controller might trigger a bus error
if BAT is 010 (data read), PROT is 1 (user mode) and the address is in a protected memory region.
...
When the CPU is in protected mode, programs have access to less types of instructions (e.g. they
cannot write to the upper word of the address registers).

Exposing this as an external pin allows other devices such as memory controllers to block access
to protected regions of memory by raising bus protection errors.
...
When low, the CPU will stop executing instructions.

It will complete the current instruction and halt before the next instruction fetch.
It will resume as normal when the input goes high.

This pin is useful for hardware driven debugging, as the CPU can be halted and the state of the
system examined.
...
When low, the Trace Mode (T) flag of the status register is forced to be 1, regardless of whether
a program has cleared it or not.
...
When low, a hardware exception will be raised by the CPU for the corresponding exception level.

They can be "masked" by the Hardware Interrupt Enable Bits (E1-E4) of the status register, which
will disable them.

If an exception for the interrupt level or higher is already being handled, the exception will be
"queued" and handled whenever all the higher level interrupts have been serviced.

If a lower level interrupt is being handled, it will be interrupted and the higher level interrupt
will be serviced.

If an interrupt pin is held low continuously it will be serviced again as soon as the exception
has been handled.
...
\begin{itemize}
	\item \textbf{Direction:} Input
	\item \textbf{Signal Type:} Falling Edge
\end{itemize}

When transitioning from high to low, will raise a level 5 hardware exception.

This interrupt cannot be masked, and if this pin is asserted while another level 5 exception is
being handled, a "level five hardware exception conflict" fault will be raised.

If asserted again while that fault is being handled, it will trigger a double fault.

Therefore, it is important that the level 5 exception handlers are executed quickly and that the NMI
is only asserted for important non-interruptible events.
...
When low, the CPU will immediately be interrupted and stop processing. It will be set to cycle 0 and
the cause register will be set to 0x1B00 (RSET).

The RSTO pin will also be asserted.

When this pin becomes high, the CPU will wait 6 cycles, stop asserting the RSTO pin, and the the
exception co-processor will execute the RSET instruction, resetting the CPU state and fetching the
reset vector.

If your system needs external devices to be reset for longer than 6 cycles to allow the program,
to run, the resets will have to be managed externally to the CPU and RSTI will need to be asserted
for however long is needed for the other devices to reset.
...
Asserted while the RSTI pin is asserted, and for 6 cycles after the RSTI pin is no longer asserted.

It will also be asserted for 6 cycles after a software reset

A reset can be triggered by asserting the Reset input (RSTI) pin, or in software by a special
exception co-processor meta instruction (RSET).

This pin is useful to reset other devices on the bus whenever the CPU is reset or to trigger more
complex reset/boot controllers.


\subsection{Timing Notes}

BACK, BERR and BPER are all mutually exclusive. If they happen to be asserted by the bus at the same
time, BERR will take first priority, then BPER, then BACK.
