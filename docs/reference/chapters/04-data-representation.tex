...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		...
		\midrule
		\reg{ah:al}         & \texttt{0x0012}        & \texttt{0x3456}       & \addr{0x123456}            \\
		\reg{ah:al}         & \texttt{0xAB12}        & \texttt{0x3456}       & \addr{0x123456}            \\
		\reg{ph:pl}         & \texttt{0x0000}        & \texttt{0x0100}       & \addr{0x000100}            \\
		\bottomrule
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{lll}
		...
		\midrule
		\addr{0x123456}         & \texttt{0x0012}            & \texttt{0x3456}              \\
		\addr{0xFECAFE}         & \texttt{0x00FE}            & \texttt{0xCAFE}              \\
		\bottomrule
	\end{tabular}
...
Instruction fetch reads the high word from the address in \reg{p} and the low word from that address plus one. \reg{p}
itself is not modified during the fetch; it advances by two words during instruction decode, so an instruction that
reads or uses \reg{p} as an address register sees the address of the instruction itself. Instruction addresses must be
even word addresses. Fetching from an odd word address raises an alignment fault.
...
Table~\ref{tab:stored-address-format}. The vector table begins at word address \addr{0x000000}, so vector entry
\texttt{V} starts at word address \texttt{V * 2}:
...
For example, vector \texttt{0x06} occupies word addresses \addr{0x00000C} and \addr{0x00000D}. To jump to
\addr{0x123456}, those two words contain \texttt{0x0012} and \texttt{0x3456}.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llX}
		...
		Memory displacements           & 16 bits        & Two's-complement displacement added to the low address word; see Section~\ref{sec:arithmetic-and-address-wraparound} for the overflow trap \\
...
for compact non-negative constants and bit masks, not for encoding negative arithmetic operands. A value that cannot be
represented in the selected immediate field width is truncated to that width; the reference assembler does not diagnose
this truncation except for the short-immediate-with-shift form, which is range-checked.
...
\section{Software Layout Conventions}

The architectural storage order for multi-word quantities is high word first. The SIRC-1 reference manual does not
otherwise define a complete ABI, stack-frame layout, argument-passing convention, or software integer library format.
Where examples show structures, arrays, saved registers, or stack use, offsets are word offsets and each push or pop
moves the stack pointer by one 16-bit word.

Software standards may build on the architectural high-word-first rule for wider integers and stored addresses, but
those conventions are non-normative unless a separate ABI or toolchain document says otherwise.

\section{Arithmetic and Address Wraparound}
\label{sec:arithmetic-and-address-wraparound}
...
Address calculation for memory, effective-address, branch, jump, and auto-update operations adds displacements to the
low word of the selected address register pair. The high address-register word is not changed by ordinary displacement,
pre-decrement, or post-increment calculation. If the low word overflows or underflows and \texttt{SR.A} (Trap on
Address Overflow)\footnote{\texttt{SR.\textit{X}} denotes bit or field \textit{X} of the status register; see
Chapter~\ref{ch:status-register} for the bit layout and Chapter~\ref{ch:reading-instructions} for the full notation
convention.} is set, the instruction raises a segment-overflow fault. If the bit is clear, the low word wraps
modulo $2^{16}$. Displacements are signed two's-complement, and the trap fires only when the low-word calculation
wraps, whether the displacement is positive or negative.
...
alignment fault. If the program counter wraps the low word while \texttt{SR.A} is set, a segment-overflow fault is
raised at the next instruction fetch, not on the instruction that caused the wrap.
