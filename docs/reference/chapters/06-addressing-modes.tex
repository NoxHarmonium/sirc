...
\begin{table}[H]
	...
	\begin{tabular}{lp{8cm}}
		...
		Implied            & No operand specified (addressing mode inferred by the assembler) \\
		\bottomrule
	\end{tabular}
...
Note: Implied is not an addressing mode supported by the CPU itself, but it is included as the assembler uses it for
meta instructions.
...
signed or unsigned value. For short immediate mode, it is an 8-bit value that allows for the instruction source to be
shifted.
...
The 8-bit or 16-bit value encoded into the instruction.
...
%TODO: Fix up all the examples of STOR where the indirect addressing is on the left side, it should be the right side
...
; Stack frame access
STOR (#-2, s), r4     ; Save local variable
...
\begin{lstlisting}
LOAD r1, (r2, a)+     ; Load from (a + r2), then a = a + 1
STOR (#2, s)+, r3     ; Store to (s + 2), then s = s + 1
\end{lstlisting}
