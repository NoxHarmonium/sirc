...
\end{verbatim}
	...
	Adds two values and stores the result in the destination register. The carry flag is set if an unsigned overflow occurs
...
\end{verbatim}
	...
	Adds two values plus the carry flag and stores the result. This instruction is used for multi-precision (32-bit,
...
\end{verbatim}
	...
	Subtracts the second operand from the first and stores the result. The carry flag is cleared if a borrow occurs
...
\end{verbatim}
	...
	Subtracts the second operand and borrow (inverse of carry) from the first operand. Used for multi-precision subtraction
...
\end{verbatim}
	...
	Performs bitwise OR operation. Each bit in the result is 1 if either corresponding bit in the operands is 1. Commonly
...
\end{verbatim}
	...
	are different. Used for toggling bits, bitwise comparison, and simple encryption.
...
\end{verbatim}
	...
	Performs subtraction but discards the result, only updating the status flags. This is the primary instruction for
...
\end{verbatim}
	...
	Performs bitwise AND but discards the result, only updating flags. Used to test if specific bits are set without
...
\end{verbatim}
	...
	Performs bitwise XOR but discards the result, only updating flags. Used to test if bits match a specific pattern or to
