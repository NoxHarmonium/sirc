...
When no override is written, ALU instructions use \texttt{[A]}. \mnemonic{SHFT} defaults to \texttt{[S]} instead, and
\mnemonic{LOAD} always encodes \texttt{[N]} and never updates the status flags, regardless of any override syntax.
Instructions that do not support status updates must not use status update override syntax.
