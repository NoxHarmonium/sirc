...
ADDI rD, #imm8, shift          ; rD = rD + (imm8 << shift)
ADDR rD, rS1, rS2              ; rD = rS1 + rS2
ADDR rD, rS1, rS2, shift       ; rD = rS1 + (rS2 << shift)
\end{lstlisting}
...
Adds two values and stores the result in the destination register. The carry flag is set if an unsigned overflow occurs (carry out of bit 15). The overflow flag is set if a signed overflow occurs (the sign of the result is incorrect for the operation).
...
ADCI rD, #imm8, shift          ; rD = rD + (imm8 << shift) + C
...
Adds two values plus the carry flag and stores the result. This instruction is used for multi-precision (32-bit, 48-bit, etc.) arithmetic to propagate the carry from lower-order additions to higher-order additions.
...
SUBI rD, #imm8, shift          ; rD = rD - (imm8 << shift)
...
Subtracts the second operand from the first and stores the result. The carry flag is cleared if a borrow occurs (operand2 > operand1 for unsigned). The overflow flag indicates signed overflow.
...
Subtracts the second operand and borrow (inverse of carry) from the first operand. Used for multi-precision subtraction to propagate borrows from lower-order to higher-order subtractions.
...
ANDI rD, #imm8, shift          ; rD = rD & (imm8 << shift)
...
Performs bitwise AND operation. Each bit in the result is 1 only if the corresponding bits in both operands are 1. Commonly used for bit masking and clearing specific bits.
...
ORRI rD, #imm8, shift          ; rD = rD | (imm8 << shift)
...
Performs bitwise OR operation. Each bit in the result is 1 if either corresponding bit in the operands is 1. Commonly used for setting specific bits.
...
XORI rD, #imm8, shift          ; rD = rD ^ (imm8 << shift)
...
Performs bitwise XOR (exclusive OR) operation. Each bit in the result is 1 if the corresponding bits in the operands are different. Used for toggling bits, bitwise comparison, and simple encryption.
...
; Load with shift
LOAD r3, #1, LSL #8        ; r3 = 256

; Zero a register
LOAD r4, #0                ; r4 = 0

; Load large constant (combine)
LOAD r5, #0x12, LSL #8     ; r5 = 0x1200
...
CMPI rS, #imm8, shift          ; Compare rS with (imm8 << shift)
...
Performs subtraction but discards the result, only updating the status flags. This is the primary instruction for implementing conditional execution and branches. The flags can be tested with condition codes to determine the relationship between the operands.
...
TSAI rS, #imm8, shift          ; Test rS & (imm8 << shift)
...
Performs bitwise AND but discards the result, only updating flags. Used to test if specific bits are set without modifying the register. The zero flag indicates whether any tested bits were set.
...
TSXI rS, #imm8, shift          ; Test rS ^ (imm8 << shift)
...
Performs bitwise XOR but discards the result, only updating flags. Used to test if bits match a specific pattern or to compare values for equality. Z=1 indicates the values are identical.
