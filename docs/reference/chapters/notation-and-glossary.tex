...
\begin{table}[H]
	...
	\begin{tabular}{lp{9cm}}
		...
		\midrule
		\reg{r1}, \reg{sr}, \reg{ph} & Register names appear in typewriter font via \texttt{\textbackslash reg\{\}}, whether
		general-purpose, address-pair, or status.                                                                              \\
		\opcode{1A}                 & A literal instruction-opcode byte value, via \texttt{\textbackslash opcode\{\}}. Renders
		as \texttt{0x1A}; the macro takes just the two hex digits.                                                             \\
		\imm{123}                   & A concrete immediate value written as an operand, via \texttt{\textbackslash imm\{\}}.
		Renders as \texttt{\#123}.                                                                                             \\
		\addr{0x0002}               & A concrete memory address or vector target, via \texttt{\textbackslash addr\{\}}.        \\
		\mnemonic{ADDI}             & An instruction mnemonic, always bold typewriter and always uppercase, via
		\texttt{\textbackslash mnemonic\{\}}.                                                                                  \\
