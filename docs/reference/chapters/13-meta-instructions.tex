...
\section{Additional Pseudo-Instructions}

Assemblers may provide additional pseudo-instructions for convenience:

\subsection{MOV -- Move}
\begin{lstlisting}
MOV rD, rS                    ; Assembles to: LOAD rD, rS
\end{lstlisting}

\subsection{CLR -- Clear Register}
\begin{lstlisting}
CLR rD                        ; Assembles to: LOAD rD, #0
\end{lstlisting}

\subsection{NOT -- Bitwise NOT}
\begin{lstlisting}
NOT rD                        ; Assembles to: XORI rD, #0xFFFF
\end{lstlisting}

\subsection{NEG -- Negate (Two's Complement)}
\begin{lstlisting}
NEG rD                        ; Assembles to: SUBR rD, #0, rD
\end{lstlisting}
