...
\begin{table}[H]
	...
	\begin{tabular}{lp{9cm}}
		...
		Exceptions             & ALU instructions do not access memory and do not raise bus, bus-protection, or alignment faults. Processing-unit encodings outside the documented ALU forms are reserved or architecturally undefined; they are not required to raise an invalid-opcode fault. \\
		\bottomrule
	\end{tabular}
...
flags are preserved. If \texttt{[S]} is encoded, flags are updated from the shifter result instead of the ALU result;
see Chapter~\ref{ch:shift-operations} for the shifter flag rules.
