...
\begin{table}[H]
	...
	\begin{tabular}{llll}
		...
		ADC                  & \texttt{ADCR rD, rS2[, shift]}      & \opcode{31}       & Writes result (shorthand: rS1 = rD) \\
		...
		SUB                  & \texttt{SUBR rD, rS2[, shift]}      & \opcode{32}       & Writes result (shorthand: rS1 = rD) \\
		...
		SBC                  & \texttt{SBCR rD, rS2[, shift]}      & \opcode{33}       & Writes result (shorthand: rS1 = rD) \\
		...
		AND                  & \texttt{ANDR rD, rS2[, shift]}      & \opcode{34}       & Writes result (shorthand: rS1 = rD) \\
		...
		ORR                  & \texttt{ORRR rD, rS2[, shift]}      & \opcode{35}       & Writes result (shorthand: rS1 = rD) \\
		...
		XOR                  & \texttt{XORR rD, rS2[, shift]}      & \opcode{36}       & Writes result (shorthand: rS1 = rD) \\
...
\subsection{Flag Computation Formulas}

Table~\ref{tab:alu-status-flag-effects} states which flags each family updates; this section states exactly how,
as a Boolean formula in terms of the sign bit (bit 15) of the destination operand (\texttt{Dm}), the source
operand (\texttt{Sm}), and the result (\texttt{Rm}). $\cdot$ is Boolean AND, $+$ is Boolean OR, and $\overline{X}$
...
\begin{table}[H]
	...
	\begin{tabular}{lp{4.2cm}p{4.2cm}}
		\toprule
		\textbf{Family}        & \textbf{C}                                                                 & \textbf{V}                                                                 \\
		\midrule
		ADD, ADC               & Carry out of bit 15 of the full addition                                  & $Dm \cdot Sm \cdot \overline{Rm} + \overline{Dm} \cdot \overline{Sm} \cdot Rm$ \\
		SUB, SBC, CMP          & $\overline{Dm} \cdot Sm + Sm \cdot Rm + Rm \cdot \overline{Dm}$            & $Dm \cdot \overline{Sm} \cdot \overline{Rm} + \overline{Dm} \cdot Sm \cdot Rm$ \\
		AND, ORR, XOR, TSA, TSX & $0$ (always cleared)                                                      & $0$ (always cleared)                                                       \\
		\bottomrule
	\end{tabular}
	\caption{Carry and Overflow Flag Formulas}
	\label{tab:alu-flag-formulas}
\end{table}

$N$ and $Z$ are the same for every family: $N = Rm$ (the result's own sign bit), and $Z$ is set when the full
16-bit result is zero. \mnemonic{ADC} and \mnemonic{SBC} use the same formulas as \mnemonic{ADD} and
\mnemonic{SUB} respectively -- the incoming carry/borrow is folded into the addition or subtraction before
\texttt{Rm} (and the carry/borrow out) is evaluated, so no separate formula is needed for them.
\mnemonic{CMP}/\mnemonic{TSA}/\mnemonic{TSX} compute the same flags as the corresponding
\mnemonic{SUB}/\mnemonic{AND}/\mnemonic{XOR} family and simply discard the result.
...
BRAN|>= @branch_if_r2_gte_r3  ; Use flags from CMPR
	\end{lstlisting}
	\caption{Manual Status Flag Update Control}
\end{example}

\begin{instructionbox}{SHFT}{Shift with Status Update Meta-Instruction}
	\textbf{Applicability:} SIRC-1, all revisions

	\textbf{Assembles to:} \texttt{ORRI[S] rD, \#0, shift}. See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
BRAN|== @zero_result          ; Branch if shifted result is zero
		\end{lstlisting}
		\caption{Shifting and Testing with SHFT}
	\end{example}
...
\begin{instructionbox}{ADDI / ADDR}{Add Immediate / Add Register}
	...
	\textbf{Opcodes:} \opcode{00} (Imm), \opcode{20} (Short Imm), \opcode{30} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
ADDI r1, #1, LSL #12       ; r1 = (r1 << 12) + 1
		\end{lstlisting}
		\caption{Basic and Scaled Addition}
	\end{example}
...
\begin{instructionbox}{ADCI / ADCR}{Add with Carry}
	...
	\textbf{Opcodes:} \opcode{01} (Imm), \opcode{21} (Short Imm), \opcode{31} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
ADCR r1, r3, r5            ; r1 = r3 + r5 + carry
		\end{lstlisting}
		\caption{32-bit Addition with Carry Propagation}
	\end{example}
...
\begin{instructionbox}{SUBI / SUBR}{Subtract Immediate / Subtract Register}
	...
	\textbf{Opcodes:} \opcode{02} (Imm), \opcode{22} (Short Imm), \opcode{32} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
BRAN|!= @loop              ; Branch if not zero
		\end{lstlisting}
		\caption{Basic Subtraction and Loop Counting}
	\end{example}
...
\begin{instructionbox}{SBCI / SBCR}{Subtract with Carry}
	...
	\textbf{Opcodes:} \opcode{03} (Imm), \opcode{23} (Short Imm), \opcode{33} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
SBCR r1, r3, r5            ; r1 = r3 - r5 - borrow
		\end{lstlisting}
		\caption{32-bit Subtraction with Borrow Propagation}
	\end{example}
...
\begin{instructionbox}{ANDI / ANDR}{AND Immediate / AND Register}
	...
	\textbf{Opcodes:} \opcode{04} (Imm), \opcode{24} (Short Imm), \opcode{34} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
BRAN|!= @common_bits       ; Branch if any bits in common
		\end{lstlisting}
		\caption{Bit Masking and Clearing with AND}
	\end{example}
...
\begin{instructionbox}{ORRI / ORRR}{OR Immediate / OR Register}
	...
	\textbf{Opcodes:} \opcode{05} (Imm), \opcode{25} (Short Imm), \opcode{35} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
\begin{instructionbox}{XORI / XORR}{XOR Immediate / XOR Register}
	...
	\textbf{Opcodes:} \opcode{06} (Imm), \opcode{26} (Short Imm), \opcode{36} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
XORR r1, r1, r2            ; r1 = (r1 ^ r2) ^ r1 = r2
		\end{lstlisting}
		\caption{Bit Toggling, Equality Testing, and XOR Swap}
	\end{example}
...
\begin{instructionbox}{LOAD}{Load/Move}
	...
	\textbf{Opcodes:} \opcode{07} (Imm), \opcode{37} (Reg). Opcode \opcode{27} is undocumented. See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
ORRI r5, #0x34             ; r5 = 0x1234
		\end{lstlisting}
		\caption{Loading Constants and Copying Registers}
	\end{example}
...
\begin{instructionbox}{CMPI / CMPR}{Compare Immediate / Compare Register}
	...
	\textbf{Opcodes:} \opcode{0A} (Imm), \opcode{2A} (Short Imm), \opcode{3A} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
; r1 is in range [10, 20)
		\end{lstlisting}
		\caption{Comparisons and Range Checking}
	\end{example}
...
\begin{instructionbox}{TSAI / TSAR}{Test AND Immediate / Test AND Register}
	...
	\textbf{Opcodes:} \opcode{0C} (Imm), \opcode{2C} (Short Imm), \opcode{3C} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
\begin{instructionbox}{TSXI / TSXR}{Test XOR Immediate / Test XOR Register}
	...
	\textbf{Opcodes:} \opcode{0E} (Imm), \opcode{2E} (Short Imm), \opcode{3E} (Reg). See Table~\ref{tab:alu-legal-forms} for the full family of legal forms.
...
BRAN|== @pattern_match
		\end{lstlisting}
		\caption{Equality and Pattern Testing with TSX}
	\end{example}
