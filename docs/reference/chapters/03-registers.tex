...
Figure~\ref{fig:register-programming-model} shows the complete sixteen-register programming model: the seven
general-purpose registers, both words of each of the four address register pairs, and \reg{sr}. The high word of
each address register pair, marked \textsuperscript{*}, is read-only in protected mode: a protected-mode program can
read its real value but a direct write faults. \reg{sr}'s high byte, marked \textsuperscript{\dag}, is stricter
still: in protected mode it is hidden even from reads, which are redacted to zero, on top of being unwritable. Its
low byte, shown in italics, is the one truly unrestricted half of \reg{sr} and remains fully accessible in both
modes. See Section~\ref{sec:privilege-restrictions} below for the full read/write privilege rules.
...
\begin{figure}[H]
	...
	\begin{bytefield}[bitwidth=1.1em]{16}
		...
		\bitbox{16}{\reg{lh} (link, high word)\textsuperscript{*}} \\
		\bitbox{16}{\reg{al} (address, low word)} \\
		\bitbox{16}{\reg{ah} (address, high word)\textsuperscript{*}} \\
		\bitbox{16}{\reg{sl} (stack, low word)} \\
		\bitbox{16}{\reg{sh} (stack, high word)\textsuperscript{*}} \\
		\bitbox{16}{\reg{pl} (program counter, low word)} \\
		\bitbox{16}{\reg{ph} (program counter, high word)\textsuperscript{*}} \\
		\bitbox{8}{\reg{sr} high byte\textsuperscript{\dag}} & \bitbox{8}{\textit{\reg{sr} low byte}}
	\end{bytefield}
	\caption{Register Programming Model}
	\label{fig:register-programming-model}

	\vspace{0.2cm}
	\footnotesize
	\textsuperscript{*}Read-only in protected mode: reads return the real value, but direct writes trigger a
	privilege violation fault (address-register-pair write-back is still allowed, provided it would leave this word
	unchanged). \quad \textsuperscript{\dag}Hidden in protected mode: reads are redacted to zero rather than the real
	value, and direct writes cannot modify it either. The \textit{italicized} \reg{sr} low byte is the odd one out
	among these: despite sharing a row with a hidden half, it is unprivileged and fully accessible in both modes.
\end{figure}
