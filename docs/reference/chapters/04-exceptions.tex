...
\begin{itemize}
	...
	\item \textbf{Alignment Fault:} Raised when fetching instructions if program counter points to an odd address
...
\begin{itemize}
	...
	\item \textbf{0x01-0x07:} Abort exception vectors (faults)
...
To return from an exception handler, the \texttt{RETI} (Return from Exception) instruction is used. This instruction
...
For abort exceptions (faults), \texttt{RETI} returns to the faulting instruction, allowing it to be retried. For
regular exceptions, \texttt{RETI} returns to the instruction following the one that was executing when the exception
occurred.
