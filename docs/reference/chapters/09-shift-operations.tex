...
ORRI r3, #0x0400         ; r3 |= 0x0400
	\end{lstlisting}
	\caption{Constant Multiply and Bit Set (LSL)}
\end{example}
...
ADDI r3, #0, LSR #8  ; r3 = (r3 >> 8)
	\end{lstlisting}
	\caption{Unsigned Divide and Byte Extraction (LSR)}
\end{example}
...
ADDI r1, #0, ASL #1  ; r1 = (r1 << 1) = r1 * 2 (signed)
	\end{lstlisting}
	\caption{Signed Multiply by 2 (ASL)}
\end{example}
...
ADDI r3, #0, ASR #1  ; r3 = (r3 >> 1) = (r4 + r5) / 2
	\end{lstlisting}
	\caption{Signed Divide and Averaging (ASR)}
\end{example}
...
SHFT r3, RTL #4
	\end{lstlisting}
	\caption{Rotating a Value Left (RTL)}
\end{example}
...
; Now branch on carry to test the old bit 0
	\end{lstlisting}
	\caption{Rotating a Value Right and Testing a Bit (RTR)}
\end{example}
...
ADDR r1, r2, r2, LSL #2   ; r1 = (r2 << 2) + r2 = r2 * 5
	\end{lstlisting}
	\caption{Multiplication by Constants}
\end{example}
...
ADDI r1, #0, ASR #3   ; r1 = (r1 >> 3) = r1 / 8 (signed)
	\end{lstlisting}
	\caption{Division by Powers of 2}
\end{example}
...
ORRI|CS r2, #0x8000   ; If old high bit 0 was set, insert it into low bit 15
	\end{lstlisting}
	\caption{32-bit Right Shift (Unsigned)}
\end{example}
...
ORRI|CS r2, #0x8000   ; If old high bit 0 was set, insert it into low bit 15
	\end{lstlisting}
	\caption{32-bit Right Shift (Signed)}
\end{example}
...
ADDI r2, #1, LSL #2           ; r2 = (r2 << 2) + 1
	...
	\end{lstlisting}
	\caption{Default Behavior (ALU flags)}
\end{example}
