...
The 4-bit condition code field in each instruction specifies under what conditions the instruction should execute. If the condition is not met, the instruction behaves as a \mnemonic{NOOP} (no operation).
...
\begin{description}
    ...
    \item[<0 (Negative)] Executes when N = 1. Tests if result is negative (signed).

    \item[>=0 (Non-Negative)] Executes when N = 0. Tests if result is positive or zero (signed).

    \item[VS (Overflow Set)] Executes when V = 1. Signed overflow occurred.

    \item[VC (Overflow Clear)] Executes when V = 0. No signed overflow.
\end{description}
...
\begin{description}
    \item[> (Unsigned Higher)] Executes when C = 1 AND Z = 0. Tests if first operand $>$ second operand (unsigned).

    \item[<= (Unsigned Lower or Same)] Executes when C = 0 OR Z = 1. Tests if first $\leq$ second (unsigned).
\end{description}

\textbf{Note:} For unsigned comparisons:
\begin{itemize}
    \item Use \texttt{>} and \texttt{<=} after \mnemonic{CMP}
    \item \texttt{>=} (unsigned) is equivalent to \texttt{CS}
    \item \texttt{<} (unsigned) is equivalent to \texttt{CC}
\end{itemize}
...
\begin{description}
    \item[Never (NV)] Never executes; equivalent to \mnemonic{NOOP}. Can be used for instruction padding or to temporarily disable instructions during debugging.
\end{description}

\section{Assembly Syntax}

Condition codes are specified by appending a pipe character (\texttt{|}) followed by the condition mnemonic to the instruction:
...
ADDR|> r3, r4, #0    ; r3 = r4 if r1 > r2 (unsigned)
...
ADDR|> r3, r1, #0        ; r3 = a if a > b
ADDR|<= r3, r2, #0       ; r3 = b if a <= b
\end{lstlisting}
...
CMPI r1, #0
LJSR|>0 (#0, a)          ; Call function if r1 > 0
\end{lstlisting}
...
loop:
    ; ... loop body ...
    SUBI r1, #1          ; i--
    BRAN|> loop          ; Continue if i > 0 (unsigned)
\end{lstlisting}
...
If a $>$ b (unsigned) & \texttt{CMPR a, b} & \texttt{>} \\
If a $\geq$ b (unsigned) & \texttt{CMPR a, b} & \texttt{CS} \\
If a $<$ b (unsigned) & \texttt{CMPR a, b} & \texttt{CC} \\
If a $\leq$ b (unsigned) & \texttt{CMPR a, b} & \texttt{<=} \\
If a $>$ b (signed) & \texttt{CMPR a, b} & \texttt{>>} \\
If a $\geq$ b (signed) & \texttt{CMPR a, b} & \texttt{>=} \\
If a $<$ b (signed) & \texttt{CMPR a, b} & \texttt{<} \\
If a $\leq$ b (signed) & \texttt{CMPR a, b} & \texttt{<<} \\
If negative & \texttt{CMPI a, \#0} & \texttt{<0} \\
If positive or zero & \texttt{CMPI a, \#0} & \texttt{>=0} \\
If overflow occurred & After arithmetic & \texttt{VS} \\
If no overflow & After arithmetic & \texttt{VC} \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    \item \textbf{Signed vs. Unsigned}: Using unsigned conditions (\texttt{>}, \texttt{<=}) for signed values or vice versa produces incorrect results
