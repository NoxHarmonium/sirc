...
\begin{itemize}
	...
	\item \textbf{Segment Overflow Fault} \textit{(Retryable)} — Raised when a computed address would fall outside
	      ...
	      Only raised if the \texttt{TrapOnAddressOverflow} SR bit is set, since some programs intentionally rely on address
	      wrap-around. Also raised if the program counter overflows; the phase field of the fault metadata register distinguishes
	      the two cases.
