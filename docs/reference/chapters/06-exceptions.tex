...
\begin{itemize}
	...
	\item \textbf{Double Fault} \textit{(Exception dispatch)} — Raised when a fault occurs while another fault is
	      ...
	      The CPU jumps to vector 0x08. This is typically a last-chance handler that dumps registers and resets. If another
	      fault occurs while the CPU is already at fault level, the CPU attempts to dispatch vector 0x08 again. If the
	      double-fault vector fetch itself fails, the CPU requests reset.
...
double-fault vector fetch fails, the CPU requests reset.
...
vector fetch failure requests reset.
