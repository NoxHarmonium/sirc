...
Not all faults behave the same way with respect to the faulting instruction and the RETE return address. There are four
categories:

\begin{description}
	\item[Retryable faults] The instruction that triggered the fault is cancelled before it takes effect. The link register
	      stores the address of the faulting instruction. RETE returns to that instruction so it can be retried, typically after
	      the fault handler has corrected the condition that caused the fault: remapping or paging in a device for a bus fault,
	      adjusting a permission for a bus protection fault, or bringing an out-of-range address back into range for a segment
	      overflow. This is also why Privilege Violation is retryable: the CPU guarantees that the instruction that triggered it
	      never took effect, so a supervisor-mode handler can safely inspect it, emulate the privileged effect if appropriate, and
	      either retry it or advance past it under its own control.

	      \textit{Alignment Fault; Bus Fault; Bus Protection Fault; Privilege Violation Fault; Segment Overflow Fault when raised
		      by program-counter wraparound.}

	\item[Aborted, non-retryable faults] The instruction that triggered the fault does not complete, but the link register stores
	      the address of the \textit{next} instruction, not the faulting one. RETE does not return to the faulting instruction,
	      so the aborted operation is not retried automatically; a handler that needs to resume it must locate and re-issue it
	      itself.

	      \textit{Invalid Opcode Fault; Segment Overflow Fault when raised by any address computation other than
...
\begin{itemize}
	...
	\item \textbf{Segment Overflow Fault} \textit{(Conditionally retryable)} — Raised when a computed address would fall
	      ...
	      metadata register help identify where the overflow occurred. When the program counter itself wraps, the link register
	      stores the address of the faulting instruction and the fault is retryable. For any other computed-address overflow, the
	      link register stores the address of the next instruction and the fault is not retryable.

	\item \textbf{Invalid Opcode Fault} \textit{(Aborted, non-retryable)} — Raised when a coprocessor call targets a
	      ...
	      will trap on older hardware, and the handler can emulate the operation in software before returning to the instruction
	      following the trapped call.
	...
	\item \textbf{Double Fault} \textit{(Exception dispatch)} — Raised when a fault occurs while another fault is
	      ...
	      If the double-fault vector fetch itself fails, the CPU requests reset. If two fault conditions are detected within the
	      same instruction before either has been dispatched, the resulting behavior is architecturally undefined.
...
the current program counter and status register values. For faults in the retryable category (Alignment Fault, and
Segment Overflow Fault raised by program-counter wraparound), the return address is the address of the faulting
instruction, so it can be retried. For all other faults and for regular exceptions (hardware and software exceptions),
...
A valid way of handling this is to declare the CPU state invalid, perform some last-chance error handling (for example,
...
:fault_handler
    ...
    ETFR #6                   ; Save fault link register (register 6) to a and r7
    STOR -(#0, s), r7
    STOR -(#0, s), ah
    STOR -(#0, s), al
    ETFR #7                   ; Save fault metadata register (register 7) to a and r7
    STOR -(#0, s), r7
    STOR -(#0, s), ah
    STOR -(#0, s), al
    ...
    LOAD ah, (#0, s)+
    LOAD r7, (#0, s)+
    ETTR #7                  ; Restore fault metadata register (register 7) from a and r7
    LOAD al, (#0, s)+
    LOAD ah, (#0, s)+
    LOAD r7, (#0, s)+
    ETTR #6                  ; Restore fault link register (register 6) from a and r7
...
This sequence restores the return address and status register of link registers 6 and 7 but not their saved exception
level, which software cannot access. A handler that has taken a double fault therefore cannot fully reconstruct the
pre-fault exception level; treat the CPU state as unrecoverable and reset.

However, if the stack pointer is invalid in this case, the double fault still corrupts the CPU state. A more robust
...
\begin{table}[H]
	\centering
	\begin{tabularx}{\textwidth}{cl>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}X}
		\toprule
...
The \mnemonic{RETE} (Return from Exception) instruction restores the saved state from the link register:
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.2cm}>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{1.1cm}>{\raggedright\arraybackslash}p{1.9cm}>{\raggedright\arraybackslash}p{1.7cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{1.2cm}}
		...
		\texttt{0x03}                & \addr{0x0006}--\addr{0x0007} & 7                 & Segment overflow & \mbox{SR.A gated}    & Faulting instr. (PC wrap only), else next & Fault metadata    & Yes             \\
		\texttt{0x04}                & \addr{0x0008}--\addr{0x0009} & 7                 & Invalid opcode   & Not maskable         & Next instruction                          & Fault metadata    & Yes             \\
...
To return from an exception handler, the \mnemonic{RETE} (Return from Exception) instruction is used. It restores the
saved state from the current link register using the same steps described under "Returning from Exceptions" in
Section~\ref{sec:exception-entry-side-effects}: it loads the return address into the program counter, restores the
saved status register (including the interrupt enable bits and, conditionally, \texttt{SR.EA}), restores the saved
exception level into \texttt{current\_exception\_level}, and resumes execution at the return address.

Executing \mnemonic{RETE} while the current exception level is 0 is architecturally undefined.

For faults in the retryable category (Alignment Fault, and Segment Overflow Fault raised by program-counter
wraparound), \mnemonic{RETE} returns to the faulting instruction, allowing it to be retried. For all other faults and
for regular exceptions, \mnemonic{RETE} returns to the instruction following the one that was executing when the
exception occurred.
