...
Control flow instructions modify the program counter to change the execution flow. The SIRCULAR provides several types of control flow operations:
...
Branches to a new location by adding a displacement to the program counter. The displacement can be an immediate value (for fixed jumps) or a register value (for computed jumps). Branches are typically PC-relative.
...
Calls a subroutine by saving the return address to the link register (\reg{l}) and jumping to the target address. The return address can later be restored using \mnemonic{RETS}.
